...
var FRBView = widgets.DOMWidgetView.extend({
    render: function() {
        return _yt_tools.then(function(yt_tools) {
            ...
            this.model.on('change:width', this
                .width_changed, this);
            this.model.on('change:height', this
                .height_changed, this);
            this.model.on('change:width', this
                .buffer_changed, this);
            this.model.on('change:height', this
            ...
            this.view_center = this.model.get(
            'view_center');
            this.model.on('change:view_width', this
                .buffer_changed, this);
            this.model.on('change:view_center', this
                .buffer_changed, this);
            this.mouse_events();
            bounds = this.calculate_view_bounds();
            this.frb = new yt_tools.FixedResolutionBuffer(
                this.model.get('width'), this.model.get(
                    'height'), bounds[0], bounds[1],
                bounds[2], bounds[3]);
            this.varmesh = new yt_tools.VariableMesh(this
                .model.get("px").data, this.model.get(
                    "py").data, this.model.get("pdx")
                .data, this.model.get("pdy").data, this
                .model.get("val").data);
            this.frb.deposit(this.varmesh, this.buffer);
            this.colormaps.data_array = this.buffer;
            this.imageData = this.ctx.createImageData(this
                .model.get('width'), this.model.get(
                    'height'), );
            this.imageData.data.set(this.colormaps
                .image_array);
    ...
    redrawCanvasImage: function() {
        ...
        createImageBitmap(this.imageData, 0, 0, nx, ny).then(
            function(bitmap) {
    ...
    mouse_events: function() {
        this.dragging = false;
        this.canvas.addEventListener('dblclick', this.onClick.bind(
            this), false);
        this.canvas.addEventListener('mousedown', this.onDownClick
            .bind(this), false);
        this.canvas.addEventListener('mouseup', this.onUpClick.bind(
            this), false);
        this.canvas.addEventListener('mousemove', this.onMove.bind(
            this), false);
    },
    onClick: function(e) {
        var loc = {
    ...
    onDownClick: function(e) {
        ...
        var cbounds = this.canvas.getBoundingClientRect();
        this.startloc.x = ((e.clientX - cbounds.left) / cbounds
            .width) * this.view_width[0];
        this.startloc.y = ((cbounds.bottom - e.clientY) / cbounds
            .height) * this.view_width[1];
    ...
    onMove: function(e) {
        if (this.dragging) {
            ...
            var cbounds = this.canvas.getBoundingClientRect();
            moveloc.x = ((e.clientX - cbounds.left) / cbounds
                .width) * this.view_width[0];
            moveloc.y = ((cbounds.bottom - e.clientY) / cbounds
                .height) * this.view_width[1];
            ...
            this.model.set({
                'view_center': this.updated_center
            });
            this.model.save_changes();
        }
    },
    colormap_events: function() {
        ...
        this.listenTo(this.colormaps, 'change:map_name',
    function() {
        ...
        this.listenTo(this.colormaps, 'change:generation',
        function() {
            this.imageData = this.ctx.createImageData(this
                .model.get('width'), this.model.get(
                    'height'), );
