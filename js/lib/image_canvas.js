...
var ImageCanvasView = widgets.DOMWidgetView.extend({
    render: function() {
        ...
        this.imageShape[0] = this.imageShape[1] = this.imageShape[
            2] = -1;
        this.model.on('change:image_array', this
            .image_array_changed, this);
    ...
    redrawCanvasImage: function() {
        ...
        createImageBitmap(this.imageData, 0, 0, nx, ny).then(
            function(bitmap) {
                this.ctx.clearRect(0, 0, canvasWidth,
                    canvasHeight);
                this.ctx.drawImage(bitmap, 0, 0, canvasWidth,
                    canvasHeight);
            }.bind(this));
    },
    image_array_changed: function() {
        ...
        a1 = this.data.shape;
        a2 = this.imageShape;
        shapeChanged = !(a1.length == a2.length && a1.every(
            function(v, i) {
                return v === a2[i]
            }));
        if (shapeChanged) {
            // We need to reallocate our imageData and update our shape
            this.imageData = this.ctx.createImageData(this.data
                .shape[0], this.data.shape[1]);
